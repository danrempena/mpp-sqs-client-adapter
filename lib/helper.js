...
helper.get_sp_jobs_as_string = (jobs) => {
    return JSON.stringify(jobs.map(job => job.data));
};

helper.enqueue_sp_results = (jobs, functionName) => {
    jobs = helper.get_sp_jobs_as_string(jobs);
    const params = {
        QueueUrl: process.env.SQS_SPJOBS_QUEUE_URL,
        DelaySeconds: 30,
        MessageBody: jobs,
        MessageAttributes: {
            "FunctionName": {
...
helper.enqueue_job_data = (jobData, error, functionName) => {
    const spJobsQueue = [];
    jobData.map(job => {
        spJobsQueue.push({data: job, error: error});
    });
    return helper.enqueue_sp_results(spJobsQueue, functionName);
};
